...
void Proton::LogBody(const char* id, Handle<Value> data)
{
  ...
  Proton::logBody->Call(Context::GetCurrent()->Global(), 2, args);
  scope.Close(Undefined());
}
...
void RegisterModule(Handle<Object> exports)
{
  ...
  Local<Value> logVal =
      Context::GetCurrent()->Global()->Get(String::New("log"));
  if (logVal->IsUndefined()) {
    ThrowException(
        Exception::TypeError(String::New("global 'log' object is undefined")));
    return;
  }
