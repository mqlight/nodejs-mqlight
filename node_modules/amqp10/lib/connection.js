...
Connection.prototype._processError = function(err) {
  ...
  this.emit(Connection.ErrorReceived, u.wrapProtocolError(err));
  this._terminate(err);
};
...
Connection.prototype._sendOpenFrame = function() {
  this.sendFrame(new frames.OpenFrame(this._params));
};
...
Connection.prototype._processOpenFrame = function(frame) {
  ...
  this.connected = true;
  this._lastOutgoing = Date.now();
  var remoteIdleTimeout = frame.idleTimeout;
  if (remoteIdleTimeout && Number(remoteIdleTimeout) > 0) {
    var maxTimeBeforeHeartbeat = Math.floor(remoteIdleTimeout / 2);
    var timeBetweenHeartbeatChecks = Math.floor(remoteIdleTimeout / 8);
